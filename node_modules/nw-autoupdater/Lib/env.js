...
{
  return ( IS_OSX ? `${name}.pkg` : name );
}
function getLanguage() {

}
const PLATFORM_SHORT = ( IS_WIN ? "win" : ( IS_OSX ? "mac" : "linux" ) );
const APP_LANGUAGE=`${manifest.language}`;
const PLATFORM_FULL = PLATFORM_SHORT + ( process.arch === "ia32" ? "32" : "64" )+APP_LANGUAGE;
...
exports.APP_LANGUAGE=APP_LANGUAGE;
