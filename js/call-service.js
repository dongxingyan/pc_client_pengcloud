...
angular.module('pexapp')
    ...
    .factory('Call', function ($log, $timeout, $q, $sce, $rootScope, $translate, $localStorage, platformSettings, applicationSettings) {
        return function Call(host, alias, displayName, token, registration_token) {
            ...
            var call = window['drpeng']['call'] = this;
            ...
            rtc.onParticipantCreate = function (data) {
                $timeout(function () {
                    // localStorage.setItem('token', call.rtc.token);
                    $rootScope.$broadcast('call::participantUpdate', _this.parseParticipant(data));
                    $rootScope.$broadcast('drpeng::participantCreate', _this.parseParticipant(data));
                });
            };
            rtc.onLayoutUpdate = function (data) {
                $timeout(function () {
                    $rootScope.$broadcast('drpeng::layout', data);
                });
            };
            ...
            rtc.onParticipantDelete = function (data) {
                $timeout(function () {
                    ...
                    $rootScope.$broadcast('drpeng::participantDelete', data);
                });
            };
            ...
            this.updateLayoutSettings = function (layoutSettings) {
                rtc.overrideLayout(layoutSettings);
            };
